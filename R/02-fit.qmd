...
# Summarise by haul
  ...
  summarise(kg_km2 = sum(no_litter_per_km2), .by = haul_id)
...
ggsave(paste0(home, paste("/figures/map.pdf")), width = 20, height = 14, units = "cm")
```
...
fit1 <- sdmTMB(kg_km2 ~ year_f + survey,
               data = catch,
               mesh = mesh,
               anisotropy = TRUE,
...
fit2 <- sdmTMB(kg_km2 ~ year_f + survey,
               data = catch,
               mesh = mesh,
               anisotropy = TRUE,
               family = delta_gamma(type = "poisson-link"),
...
fit3 <- sdmTMB(kg_km2 ~ year_f + survey,
               data = catch,
               mesh = mesh,
               anisotropy = TRUE,
               family = delta_gamma(type = "poisson-link"),
...
fit4 <- sdmTMB(kg_km2 ~ year_f + survey,
               data = catch,
               mesh = mesh,
               anisotropy = TRUE,
               family = delta_gamma(type = "poisson-link"),
...
p1 <- plot_map + 
  ...
  scale_fill_gradient2(midpoint = 1, name = "Spatiotemporal random effect (binomial model)") + 
...
p2 <- plot_map + 
  ...
  # theme(legend.position.inside = c(0.95, 0.2)) + 
  guides(fill = guide_colorbar(title.position = "top")) +
  scale_fill_gradient2(midpoint = 1, name = "Spatiotemporal random effect (Gamma model)") 

p1 / p2

ggsave(paste0(home, "/figures/spatial_random.pdf"), width = 14, height = 17, units = "cm")
...
plot_map + 
  ...
  scale_fill_viridis_c(
    ...
    na.value = "yellow", limits = c(0, quantile(exp(p$est1 + p$est2), 0.995))
  )

quantile(exp(p$est1 + p$est2), 0.995)

ggsave(paste0(home, "/figures/spatial_pred.pdf"), width = 20, height = 14, units = "cm")
...
ggplot(ind, aes(year, est, color = Survey, fill = Survey)) +
  ...
  labs(x = "Year", y = "Relative litter weight")

ggsave(paste0(home, "/figures/index.pdf"), width = 14, height = 10, units = "cm")
```
